...
var dist_2 = require("../../graphql-codegen-generators/dist");
...
function compileTemplate(config, templateContext, documents, settings) {
    ...
    dist_1.debugLog("[compileTemplate] starting to compile template with input type = " + config.inputType);
    dist_1.debugLog("[compileTemplate] settings = ", settings);
    ...
    Object.keys(templates).forEach(function (templateName) {
        dist_1.debugLog("[compileTemplate] register partial template " + templateName);
        handlebars_1.registerPartial(templateName, templates[templateName]);
    });
    var mergedDocuments;
    if (!executionSettings.generateDocuments) {
        dist_1.debugLog("[compileTemplate] generateDocuments is false, ignoring documents...");
        mergedDocuments = {
            fragments: [],
            operations: [],
            hasFragments: false,
        ...
        dist_1.debugLog("[compileTemplate] all documents merged into single document, total of " + mergedDocuments.operations.length + " operations and " + mergedDocuments.fragments.length + " fragments");
        if (config.flattenTypes) {
            dist_1.debugLog("[compileTemplate] flattenTypes is true, flattening all selection sets from all documents...");
            mergedDocuments = flatten_types_1.flattenTypes(mergedDocuments);
        }
    }
    if (config.inputType === dist_2.EInputType.SINGLE_FILE) {
        if (!templates['index']) {
        ...
        dist_1.debugLog("[compileTemplate] Executing generateSingleFile...");
    ...
    else if (config.inputType === dist_2.EInputType.MULTIPLE_FILES || config.inputType === dist_2.EInputType.PROJECT) {
        if (config.inputType === dist_2.EInputType.MULTIPLE_FILES) {
            if (!config.filesExtension) {
                throw new Error('Config filesExtension is required when using inputType = MULTIPLE_FILES!');
            }
        }
        dist_1.debugLog("[compileTemplate] Executing generateMultipleFiles...");
        var compiledTemplates = Object.keys(templates).map(function (templateName) {
            dist_1.debugLog("[compileTemplate] Compiling template: " + templateName + "...");
        ...
        dist_1.debugLog("[compileTemplate] Templates names: ", Object.keys(compiledTemplates));
